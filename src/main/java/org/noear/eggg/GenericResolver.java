...
public class GenericResolver {
    ...
    private final Map<Type, SoftReference<Map<String, Type>>> genericInfoCached = new ConcurrentHashMap<>();
    ...
    public Map<String, Type> getGenericInfo(Type type) {
        return genericInfoCached.compute(type, (k, softRef) -> {
            if (softRef != null && softRef.get() != null) {
                return softRef;
            }

            return new SoftReference<>(createTypeGenericMap(k));
        }).get();
    }
