...
class EgggReflectTest {
    ...
    void testOnClassByClass() {
        EgggReflect reflect = eggg.onClass(Person.class);
    ...
    void testOnClassByClassName() {
        String result = eggg.onClass("java.lang.String")
                .create("Hello").get();
        assertEquals("Hello", result);
    }

    @Test
    void testOnClassByClassNameNotFound() {
        EgggReflectException ex = assertThrows(EgggReflectException.class, () ->
                eggg.onClass("com.nonexistent.Class"));
        assertTrue(ex.getCause() instanceof ClassNotFoundException);
    }

    @Test
    void testOnClassNull() {
        assertThrows(NullPointerException.class, () -> eggg.onClass((Class<?>) null));
    }

    @Test
    void testOnBeanWithObject() {
        Person person = new Person("Tom");
        EgggReflect reflect = eggg.onBean(person);
        assertEquals(Person.class, reflect.type());
    ...
    @Test
    void testOnBeanNull() {
        assertThrows(NullPointerException.class, () -> eggg.onBean(null));
    }

    @Test
    void testOnBeanTypePreserved() {
        assertEquals(String.class, eggg.onBean("hello").type());
    }

    @Test
    void testOnBeanWithInteger() {
        assertEquals(Integer.class, eggg.onBean(42).type());
    }
    ...
    void testCreateNoArgs() {
        Person person = eggg.onClass(Person.class).create().get();
    ...
    void testCreateWithSingleArg() {
        Person person = eggg.onClass(Person.class).create("Alice").get();
    ...
    void testCreateWithMultipleArgs() {
        Person person = eggg.onClass(Person.class).create("Tom", 25).get();
    ...
    void testCreateString() {
        assertEquals("Hello World", eggg.onClass(String.class).create("Hello World").get());
    }

    @Test
    void testCreateStringNoArgs() {
        assertEquals("", eggg.onClass(String.class).create().get());
    }

    @Test
    void testCreateWithPrimitiveArgs() {
        // Integer 传给 int 参数（模糊匹配）
        Person person = eggg.onClass(Person.class).create("Bob", Integer.valueOf(30)).get();
    ...
    void testCreateNoMatchingConstructor() {
        EgggReflectException ex = assertThrows(EgggReflectException.class, () ->
                eggg.onClass(String.class).create(1, 2, 3, 4, 5));
        assertTrue(ex.getCause() instanceof NoSuchMethodException);
    }

    @Test
    void testCreateReturnsNewReflect() {
        EgggReflect reflect = eggg.onClass(Person.class).create("Test");
    ...
    void testCreateChainedWithCall() {
        String result = eggg.onClass(Person.class)
                .create("Tom")
                .call("hello")
                .get();
    ...
    void testCreateOnlyParameterizedConstructor() {
        Book book = eggg.onClass(Book.class)
    ...
    void testCreateMultipleTimes() {
        EgggReflect classReflect = eggg.onClass(Person.class);
    ...
    void testCallNoArgs() {
        assertEquals("Hello, I'm Tom",
                eggg.onBean(new Person("Tom", 25)).call("hello").get());
    }

    @Test
    void testCallWithArgs() {
        assertEquals("Hi, Tom!",
                eggg.onBean(new Person("Tom", 25)).call("greet", "Hi").get());
    }

    @Test
    void testCallWithPrimitiveArg() {
        Person person = new Person();
        eggg.onBean(person).call("setAge", 25);
        assertEquals(25, person.getAge());
    }
    ...
    void testCallVoidMethod() {
        Person person = new Person("Tom");
        EgggReflect reflect = eggg.onBean(person).call("voidMethod");
    ...
    void testCallVoidMethodReturnsSameObject() {
        ...
        EgggReflect reflect = eggg.onBean(service).call("increment");
    ...
    void testCallChained() {
        assertEquals("World",
                eggg.onClass(String.class).create("Hello World")
                        .call("substring", 6).call("toString").get());
    }

    @Test
    void testCallReturnsNull() {
        EgggReflect reflect = eggg.onBean(new Person()).call("getName");
        assertNull(reflect.get());
    ...
    void testCallStaticMethod() {
        assertEquals("static hello",
                eggg.onClass(Person.class).call("staticHello").get());
    }

    @Test
    void testCallMethodNotFound() {
        EgggReflectException ex = assertThrows(EgggReflectException.class, () ->
                eggg.onBean(new Person()).call("nonExistentMethod"));
        assertTrue(ex.getCause() instanceof NoSuchMethodException);
    }
    ...
    void testCallWithWrongArgCount() {
        EgggReflectException ex = assertThrows(EgggReflectException.class, () ->
                eggg.onBean(new Person()).call("greet", "Hi", "Extra"));
        assertTrue(ex.getCause() instanceof NoSuchMethodException);
    }
    ...
    void testCallWithWrongArgTypes() {
        ...
        assertThrows(EgggReflectException.class, () ->
                eggg.onBean(new Person()).call("greet", 123));
    }

    @Test
    void testCallOverloadedNoArgs() {
        assertEquals("woof", eggg.onBean(new Dog()).call("bark").get());
    }

    @Test
    void testCallOverloadedWithInt() {
        assertEquals("woof x3", eggg.onBean(new Dog()).call("bark", 3).get());
    }

    @Test
    void testCallOverloadedWithString() {
        assertEquals("hey woof", eggg.onBean(new Dog()).call("bark", "hey").get());
    }
    ...
    void testCallInheritedMethod() {
        ...
        assertEquals("Canine", eggg.onBean(dog).call("getSpecies").get());
    }

    @Test
    void testCallOverriddenMethod() {
        assertEquals("Woof!", eggg.onBean(new Dog()).call("makeSound").get());
    }

    @Test
    void testCallPrivateMethod() {
        PrivateAccess pa = new PrivateAccess();
        assertEquals("hidden", eggg.onBean(pa).call("getSecret").get());
    }

    @Test
    void testCallPrivateSetter() {
        PrivateAccess pa = new PrivateAccess();
        eggg.onBean(pa).call("setSecret", "revealed");
        assertEquals("revealed", eggg.onBean(pa).call("getSecret").get());
    }

    @Test
    void testCallAllPrimitiveTypes_boolean() {
        Calculator calc = new Calculator();
        assertTrue(eggg.onBean(calc).call("check", true).<Boolean>get());
    }

    @Test
    void testCallAllPrimitiveTypes_byte() {
        Calculator calc = new Calculator();
        assertEquals((byte) 2, (byte) eggg.onBean(calc).call("inc", (byte) 1).<Byte>get());
    }

    @Test
    void testCallAllPrimitiveTypes_short() {
        Calculator calc = new Calculator();
        assertEquals((short) 20, (short) eggg.onBean(calc).call("doubleShort", (short) 10).<Short>get());
    }

    @Test
    void testCallAllPrimitiveTypes_int() {
        Calculator calc = new Calculator();
        assertEquals(5, (int) eggg.onBean(calc).call("add", 2, 3).get());
    }

    @Test
    void testCallAllPrimitiveTypes_long() {
        Calculator calc = new Calculator();
        assertEquals(6L, (long) eggg.onBean(calc).call("multiply", 2L, 3L).get());
    }

    @Test
    void testCallAllPrimitiveTypes_float() {
        Calculator calc = new Calculator();
        assertEquals(2.5f, eggg.onBean(calc).call("half", 5.0f).<Float>get(), 0.001f);
    }

    @Test
    void testCallAllPrimitiveTypes_double() {
        Calculator calc = new Calculator();
        assertEquals(9.0, eggg.onBean(calc).call("square", 3.0).get());
    }

    @Test
    void testCallAllPrimitiveTypes_char() {
        Calculator calc = new Calculator();
        assertEquals('B', (char) eggg.onBean(calc).call("nextChar", 'A').<Character>get());
    }

    @Test
    void testCallReturnsArray() {
        CollectionHolder holder = new CollectionHolder();
        String[] arr = eggg.onBean(holder).call("getStringArray").get();
    ...
    void testCallReturnsList() {
        ...
        List<String> list = eggg.onBean(holder).call("getStringList").get();
    ...
    void testCallReturnsMap() {
        CollectionHolder holder = new CollectionHolder();
        @SuppressWarnings("unchecked")
        Map<String, Integer> map = eggg.onBean(holder).call("getMap").get();
    ...
    void testCallVoidMethodChain() {
        VoidService service = new VoidService();
        eggg.onBean(service).call("increment").call("increment");
    ...
    void testCallVoidMethodWithArgs() {
        VoidService service = new VoidService();
        eggg.onBean(service).call("add", 5);
    ...
    void testCallOverloadedIntVsInteger() {
        VoidService service = new VoidService();
        // int 参数精确匹配
        eggg.onBean(service).call("add", 10);
    ...
    void testFieldGetReturnsEgggReflect() {
        EgggReflect reflect = eggg.onBean(new Person("Tom")).field("name");
    ...
    void testFieldNotFound() {
        EgggReflectException ex = assertThrows(EgggReflectException.class, () ->
                eggg.onBean(new Person()).field("nonExistentField"));
    ...
    void testPropertyGet() {
        assertEquals("Tom", eggg.onBean(new Person("Tom")).property("name").get());
    }

    @Test
    void testPropertySet() {
        Person person = new Person();
        eggg.onBean(person).setProperty("name", "Alice").setProperty("age", 25);
        assertEquals("Alice", person.getName());
        assertEquals(25, person.getAge());
    }
    ...
    void testPropertyGetReturnsEgggReflect() {
        EgggReflect reflect = eggg.onBean(new Person("Tom")).property("name");
        assertEquals("Tom", reflect.get());
    }
    ...
    void testPropertySetReturnsThis() {
        Person person = new Person();
        EgggReflect reflect = eggg.onBean(person).setProperty("name", "Tom");
        assertSame(person, reflect.get());
    }
    ...
    void testPropertySetAndGetChain() {
        Person person = new Person();
        String name = eggg.onBean(person)
    ...
    void testPropertyReadOnly() {
        ...
        assertEquals("readonly", eggg.onBean(po).property("readOnly").get());
    }
    ...
    void testPropertyFallbackToField() {
        ...
        Person person = new Person("Tom");
        assertEquals("Tom", eggg.onBean(person).property("name").get());
    }

    @Test
    void testPropertyWithPrimitiveType() {
        Person person = new Person();
        eggg.onBean(person).setProperty("age", 30);
        int age = eggg.onBean(person).property("age").get();
        assertEquals(30, age);
    }
    ...
    void testPropertySetWithNull() {
        Person person = new Person("Tom");
        eggg.onBean(person).setProperty("name", null);
        assertNull(person.getName());
    }
    ...
    void testTypeOnClass() {
        assertEquals(Person.class, eggg.onClass(Person.class).type());
    }
    ...
    void testTypeAfterCreate() {
        assertEquals(Person.class, eggg.onClass(Person.class).create().type());
    }

    @Test
    void testTypeAfterCall() {
        assertEquals(String.class,
                eggg.onBean(new Person("Tom")).call("hello").type());
    }

    @Test
    void testTypeAfterCallNull() {
        assertEquals(String.class,
                eggg.onBean(new Person()).call("getName").type());
    }

    @Test
    void testTypeAfterField() {
        assertEquals(String.class,
                eggg.onBean(new Person("Tom")).field("name").type());
    }

    @Test
    void testGetReturnsCorrectType() {
        String result = eggg.onBean("Hello").get();
        assertEquals("Hello", result);
    }
    ...
    void testEqualsSameObject() {
        Person person = new Person("Tom");
        EgggReflect r1 = eggg.onBean(person);
        EgggReflect r2 = eggg.onBean(person);
        assertEquals(r1, r2);
    }

    @Test
    void testEqualsDifferentObject() {
        assertNotEquals(eggg.onBean(new Person("Tom")), eggg.onBean(new Person("Tom")));
    }

    @Test
    void testEqualsNull() {
        assertNotEquals(null, eggg.onBean(new Person()));
    }

    @Test
    void testEqualsNonReflect() {
        assertNotEquals("not an EgggReflect", eggg.onBean(new Person()));
    }
    ...
    void testHashCodeDifferent() {
        EgggReflect r1 = eggg.onBean("A");
        EgggReflect r2 = eggg.onBean("B");
        assertNotEquals(r1.hashCode(), r2.hashCode());
    }

    @Test
    void testToStringWithValue() {
        assertEquals("Hello", eggg.onBean("Hello").toString());
    }
    ...
    void testUserScenarioFromClass() {
        assertEquals("World",
                eggg.onClass(String.class).create("Hello World").call("substring", 6).get());
    }

    @Test
    void testUserScenarioFromBean() {
        assertEquals("World",
                eggg.onBean("Hello World").call("substring", 6).get());
    }
    ...
    void testFullChainProperty() {
        Person person = eggg.onClass(Person.class).create()
                .setProperty("name", "Alice")
    ...
    void testFullChainCalculator() {
        Calculator calc = new Calculator();
        int sum = eggg.onBean(calc).call("add", 10, 20).get();
        assertEquals(30, sum);
        long product = eggg.onBean(calc).call("multiply", 3L, 7L).get();
    ...
    void testCustomEgggInstance() {
        ...
        assertEquals("World",
                customEggg.onBean("Hello World").call("substring", 6).get());
    }
    ...
    void testMultipleEgggInstances() {
        ...
        Eggg eggg2 = new Eggg();
        assertEquals("A",
                eggg1.onBean("AB").call("substring", 0, 1).get());
        assertEquals("B",
                eggg2.onBean("AB").call("substring", 1, 2).get());
    }

    @Test
    void testCachingConsistency() {
        Person person = new Person("Tom");
        String r1 = eggg.onBean(person).call("hello").get();
        String r2 = eggg.onBean(person).call("hello").get();
        assertEquals(r1, r2);
    }
    ...
    void testReflectAfterEgggClear() {
        Eggg eggg = new Eggg();
        String r1 = eggg.onBean("Hello").call("substring", 0, 2).get();
        eggg.clear();
        String r2 = eggg.onBean("Hello").call("substring", 0, 2).get();
        assertEquals(r1, r2);
    }
    ...
    void testExceptionCauseIsNoSuchMethod() {
        EgggReflectException ex = assertThrows(EgggReflectException.class, () ->
                eggg.onBean(new Person()).call("nonExistent"));
        assertTrue(ex.getCause() instanceof NoSuchMethodException);
    }
    ...
    void testCallTargetMethodThrows() {
        ...
        assertThrows(EgggReflectException.class, () ->
                eggg.onBean("Hello").call("substring", -1));
    }
}
